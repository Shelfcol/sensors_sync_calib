...
//PoseGraph的构造函数中创建了一个4自由度位姿图优化线程
PoseGraph::PoseGraph()
{
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    //如果存在回环候选帧，即loop_index != -1：
	if (loop_index != -1)
	{
        //将当前帧与回环帧进行描述子匹配，如果成功则确定存在回环
        //获取回环候选帧
    ...
    //绘制可视化轨迹中帧间的连线，发布topic：pub_pg_path、pub_path、pub_base_path
...
//回环检测
/*
该函数用于检测当前帧与先前帧是否可能存在回环，
若存在则返回回环候选帧的索引。
在函数中使用大量DEBUG条件语句，用于在调试时对当前状态进行可视化输出
*/
