...
//额外检测500个新的特征点并计算所有特征点的描述子，为了回环检测
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	TicToc t_match;
		...
		if (DEBUG_IMAGE)    
	    {
	        ...
	        cvtColor(gray_img, loop_match_img, cv::COLOR_BGR2RGB);
	        for(int i = 0; i< (int)point_2d_uv.size(); i++)
	        {
	...
	//将关键帧与回环帧进行BRIEF描述子匹配，并剔除匹配失败的点
	...
	*/
	...
	#endif
	//如果能匹配的特征点能达到最小回环匹配个数，则用RANSAC PnP检测再去除误匹配的点
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_id, status);
	    	...
	    	if (DEBUG_IMAGE)
	        {
				//将此关键帧和回环帧拼接起来，将对应的匹配点相连以绘制回环匹配图，并发布为pub_match_img
	        	int gap = 10;
	        	cv::Mat gap_image(ROW, gap, CV_8UC1, cv::Scalar(255, 255, 255));
	            cv::Mat gray_img, loop_match_img;
	            cv::Mat old_img = old_kf->image;
