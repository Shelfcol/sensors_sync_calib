...
#include "camera_camera_sync_with_opt/camera_camera_sync_with_opt.hpp"
...
void findMatchPoints(const Mat img_left, const Mat img_right, vector<Point2f>& pts1, vector<Point2f>& pts2)
{
    ...
    {
        cvtColor(img_left, grayimg1, COLOR_BGR2GRAY);
    } else {
    ...
    if(3 == img_left.channels())
    {
        cvtColor(img_right, grayimg2, COLOR_BGR2GRAY);
    } else {
    ...
    // imshow("ORB descriptor",grayimg1);
    // cv::waitKey(0);
   ...
   //特征点描述，为下边的特征点匹配做准备  
    ...
    // 检测特征点
    orb->detect(img1, keypoints_object);
    orb->detect(img2, keypoints_scene);

    // 计算描述子
    cv::Mat matshow1, matshow2, kp1, kp2; 
    orb->compute(img1, keypoints_object, descriptors_object);
	orb->compute(img2, keypoints_scene, descriptors_scene);

    cv::Mat outImg;
    cv::drawKeypoints(img1,keypoints_object,outImg,cv::Scalar::all(-1),cv::DrawMatchesFlags::DEFAULT);
	...
	//绘制出匹配到的关键点
	cv::Mat imgMatches;
	cv::drawMatches(img1, keypoints_object, img2, keypoints_scene,
